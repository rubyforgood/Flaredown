...
RSpec.describe Ability do
  ...
  context 'for conditions' do
    ...
    context 'popular conditions' do
      before do
        create_list(:user_condition, Flaredown.config.trackables_min_popularity, condition: global_condition)
      end
      ...
      it { expect(popular_condition_ids).not_to eq([popular_condition.id]) }
    end
  end
end
